...
def detect_anomalies(df, std_multiplier=3):
    ...
    # Рассчитываем статистики для каждого пользователя
    ...
    user_stds = df.groupby('IdSubscriber').agg({
        'UpTx': 'std',
        'DownTx': 'std'
    }).fillna(0)  # Заполняем 0 там, где std невозможно посчитать
    
    # Добавляем статистики к основному DataFrame
...
async def get_statistics():
    ...
    try:
        ...
        # Ensure Start1hPeriod is in datetime format
